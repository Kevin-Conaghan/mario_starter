...
public class Player : MonoBehaviour {
    ...
    public float bounce = -4.0F;
	public float gravity = 20.0F;
	private Vector3 moveDirection = Vector3.zero;

    GameObject EnemyGameObject; // enemy object reference 
    ...
    {
        // apply negative force when hitting block to simulate mass
        if (other.collider.gameObject.CompareTag  ("PlatformBlock"))
        {
            moveDirection.y = bounce;
        }
        else if (other.collider.gameObject.CompareTag ("Fall"))
        {
            // death from falling 
             Reset();
